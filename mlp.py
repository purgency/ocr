...
InputA = np.array([None]*72)
InputB = np.array([None]*72)

#reshapes images and fills them into input arrays
for i in range(0,72):
    img_path = 'images/A/A' + str(i+1) + '.png'
    img = cv2.imread(img_path, 0)
    img = np.reshape(img, 400).astype(float)
    InputA[i] = img
    img_path = 'images/B/B' + str(i+1) + '.png'
    img = cv2.imread(img_path, 0)
    img = np.reshape(img, 400).astype(float)
    InputB[i] = img
    
#inverts and makes values uniform between 0 and 1
for x in range(0,72):
    for y in range(0,400):
        InputA[x][y] = 1 - (InputA[x][y] / 255.0)
        InputB[x][y] = 1 - (InputB[x][y] / 255.0)
        


whid = np.random.uniform(0.0, 1.0, (100,400)) #weights from input neurons to hidden layer neurons
wout = np.random.uniform(0.0, 1.0, (1,100)) #weights from hidden layer neurons to output neurons

#bhid = np.random.rand(100)
...
for x in range(10): #number of training episodes
